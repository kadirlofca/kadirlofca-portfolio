...
<div class="flex flex-col max-w-64 w-max p-dist-0 gap-8">
    ...
    <div class="p-dist-1 bg-dist-0-foreground">
        <h1 class="text-3xl text-center font-black text-dist-2-foreground">{Personal.fullName}</h1>
    </div>
    ...
    <!-- PICTURE -->
    <div class="bg-dist-0-foreground">
        <img src={Headshot} alt="Kadir Lofca." class="w-max aspect-square border border-dist-0-foreground mix-blend-luminosity" />
    </div>
...
{#snippet skillBar(title: string, percentage: number)}
    <div class="relative flex flex-row">
        <div class="flex-1 border border-dist-0-foreground">
            <PercentBar percentage={percentage} class="h-8 ml-auto bg-dist-0-foreground" />
        </div>
